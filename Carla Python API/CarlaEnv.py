...
#------------------------------------------------------------------------------------新增的 begin
    ...
    def get_front_vehicle_dist(self):
        vehi_transform = self.vehicle.get_transform()
        vehi_location = vehi_transform.location
        vehi_forward = vehi_transform.get_forward_vector()
        ...
        for vehicle in vehicles:
            ...
            vec_to_target = target_location - vehi_location
            distance = vec_to_target.length()
            
            # angle between fwd direction and vec to target
            vec_to_target_np = np.array([vec_to_target.x, vec_to_target.y, vec_to_target.z])
            vehi_forward_np = np.array([vehi_forward.x, vehi_forward.y, vehi_forward.z])
            vec_to_target_np /= np.linalg.norm(vec_to_target_np)
            vehi_forward_np /= np.linalg.norm(vehi_forward_np)
            
            dot = np.dot(vec_to_target_np, vehi_forward_np)
            
            # only within ~45 degrees cone
    ...
    def get_ahead_traffic_light(self, max_dist=40.0, max_vehicle_angle=33.3, max_facing_angle=120.0):
        """
        within `max_dist`
        within `max_vehicle_angle` of the vehicle,
        and traffic lights fwd vector is pointing toward the vehicle within `max_facing_angle`.
        """
        ...
        for tl in self.world.get_actors().filter('traffic.traffic_light*'):
            ...
            # light's forward vector pointing toward the vehicle
    ...
    def step(self, action, detections):
        ...
        if traffic_light:
            ...
            if dist_to_tl < TL_MAX_DIST and (tl_state == carla.TrafficLightState.Red or tl_state == carla.TrafficLightState.Yellow):
                red_light_flag = 1
                proximity_factor = max(0.0, (TL_MAX_DIST - dist_to_tl) / TL_MAX_DIST)

                if speed > 7.5:
                    # penalty scales
                    penalty = 20.0 * proximity_factor * (speed / 25)
                    reward -= penalty
                ...
                if speed < 5.0:
                    reward -= 5.0
                    print("Stopped at green light, bad!")
        else:
        ...
        if waypoint is not None:
            ...
            # positive too much to the right
            # negative too much to the left
            if angle_signed is not None:
                if abs(angle_signed) < 2:
                    reward += 0.1
                elif abs(angle_signed) < 10:
